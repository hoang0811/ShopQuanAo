    ...
    public function GenerateBrandThumbnailsImage($image, $imageName)
{
    ...
    // Lưu ảnh mà không thay đổi kích thước
    ...
    public function brand_update(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            // Gọi hàm xử lý ảnh thumbnail và lưu ảnh
            $this->GenerateBrandThumbnailsImage($image, $file_name);
    ...
    public function product_store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $this->GenerateProductThumbnailsImage($imageName);
        ...
        if ($request->hasFile('images')) {
            $allowedfileExtensions = ['jpg', 'png', 'jpeg','avif'];
            ...
            foreach ($files as $file) {
                ...
                if (in_array($gextension, $allowedfileExtensions)) {
                    ...
                    $this->GenerateProductThumbnailsImage($gfileName);
    ...
    public function GenerateProductThumbnailsImage($imageName)
    {
    ...
    public function product_update(Request $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach (explode(',', $product->images) as $ofile) {
                ...
                if (File::exists(public_path('uploads/products') . '/' . $product->$ofile)) {
                    File::delete(public_path('uploads/products') . '/' . $product->$ofile);
                }
                if (File::exists(public_path('uploads/products/thumbnails') . '/' . $product->$ofile)) {
                    File::delete(public_path('uploads/products/thumbnails') . '/' . $product->$ofile);
                }
            }
            ...
            foreach ($files as $file) {
                ...
                if (in_array($gextension, $allowedfileExtensions)) {
                    ...
                    // Di chuyển từng ảnh vào thư mục lưu trữ
                    $file->move(public_path('uploads/products'), $gfileName);

                    // Gọi hàm tạo thumbnail
                    $this->GenerateProductThumbnailsImage($gfileName);
    ...
    public function slide_store(Request $request)
    {
        $request->validate([
            ...
            'subtitle' => 'required',
            'link' => 'required',
            'image' => 'nullable|mimes:png,jpg,jpge|max:2048',
            'status' => 'required',
        ]);
        $slide = new Slide();
    ...
    public function slide_delete($id)
    {
        ...
        $slide->delete();
        return redirect()->route('admin.slides')->with('status', 'Brand has been delete successfully !');
    }
